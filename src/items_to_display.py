...
def display_tiles():
    ...
    fields = DbTool().get_all_rows(('src.objects.fields', 'Field'))
    for field in fields:
        blit(field_types[field.type.name], (field.y * 32, field.x * 32))


def display_creatures():
    for creature in DbTool().get_all_rows(('src.objects.creatures', 'SpawnedCreature')):
        blit(pygame.image.load(creature.type.image), (creature.x * 32, creature.y * 32))


def display_tiles_items():
    for element in DbTool().get_rows_where(("src.objects.items", "BoundedItem", "field_id"), None, equal=False):
